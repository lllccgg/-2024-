...
namespace Config {
	...
	const char* const onnx_path = "D:\\vsworkspaces\\ArmorNumClassifier_svm_model\\lenet5_armor_end.onnx"; // onnx模型路径

    int bianry = 8; // 输入模型的图像二值化阈值
	int roi_x = 30; // ROI区域x方向
	int roi_y = 45; // ROI区域y方向
	int x_offset = 65; // x方向偏移
	int y_offset = 115; // y方向偏移

	// 调节曝光
    int gamma_val = 8; // 初始伽马值（x10，例如1.8）
    int trunc_val = 220; // 初始亮度截断阈值
...
int main()
{
    ...
    cap.open("D:\\Opencvpracticedata\\NumClasiifier.mp4"); // 读取视频路径
